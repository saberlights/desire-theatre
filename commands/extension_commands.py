...
from ..extensions import OutfitSystem, ItemSystem, SceneSystem, GameSystem
...
class DTUseItemCommand(BaseCommand):
    ...
    command_description = "使用背包中的道具"
    command_pattern = r"^/(用|use)\s+(.+)$"

    async def execute(self) -> Tuple[bool, str, bool]:
        match = re.match(self.command_pattern, self.message.processed_plain_text)
        if not match:
            await self.send_text("❌ 格式错误！使用: /用 <道具名称>")
            return False, "格式错误", False
        ...
        # 使用道具 (增强版 - 带心理场景)
        success, result = await ItemSystem.use_item(user_id, chat_id, item["item_id"], character)

        if not success:
            await self.send_text(f"❌ 没有 {item_name} 或数量不足")
            return False, "道具不足", False

        # 显示心理场景描述
        if result.get("scene_description"):
            await self.send_text(result["scene_description"])

        # 合并基础效果和额外心理效果
        total_effects = result["effects"].copy()
        if result.get("extra_effects"):
            for k, v in result["extra_effects"].items():
                total_effects[k] = total_effects.get(k, 0) + v

        # 应用效果到角色
        if character:
            updated = AttributeSystem.apply_changes(character, total_effects)
            await database_api.db_save(
                DTCharacter,
                data=updated,
                key_field="user_id",
