...
from ...systems.time.daily_limit_system import DailyInteractionSystem
...
class DTNextDayCommand(BaseCommand):
    ...
    async def execute(self) -> Tuple[bool, str, bool]:
        ...
        # 【新增】每日职业成长 - 自动提升职业属性
        character, growth_messages = CareerSystem.daily_career_growth(character)

        # 【修复】立即保存职业数据，确保晋升检查时数据已持久化
        await database_api.db_save(
            DTCharacter,
            data=character,
            key_field="user_id",
        ...
        # 构建收入和成长消息
        income_msg = f"💰 【每日收入】+{daily_income}币 (余额: {character['coins']})"
        if growth_messages:
            income_msg += "\n\n📊 【职业成长】\n" + "\n".join(growth_messages)

        await self.send_text(income_msg)
        ...
        if random.random() < 0.3:
            ...
            if dynamic_event:
                # 成功生成动态事件
                event_id = f"dynamic_{int(__import__('time').time())}"

                # 【修复】存储完整事件数据到角色数据，而不仅仅是ID
                character["active_event"] = json.dumps({
                    "event_id": event_id,
                    "event_data": dynamic_event  # 完整的事件数据
                }, ensure_ascii=False)
                ...
                # 显示事件消息
                event_message = RandomEventSystem.format_event_message(dynamic_event, character)
                await self.send_text(event_message)

                logger.info(f"触发完全动态事件: {dynamic_event['event_name']}")
            else:
                # 动态生成失败，回退到预定义事件
                logger.warning("动态事件生成失败，回退到预定义事件")
                ...
                if event_tuple:
                    ...
                    # 使用 LLM 生成事件文本
                    generated_content = await RandomEventSystem.generate_event_content_with_llm(
                        event_id=event_id,
                    ...
                    if generated_content:
                        event_data = RandomEventSystem.merge_generated_content(event_data, generated_content)
                        logger.info(f"使用LLM生成的事件内容: {event_id}")
                    else:
                        logger.warning(f"LLM生成失败，使用固定文本: {event_id}")
    ...
    async def _generate_week_summary(self, character: dict, week_num: int) -> str:
        """生成周总结"""
        from ...systems.actions.action_growth_system import ActionGrowthSystem
        import json
