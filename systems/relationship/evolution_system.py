...
class EvolutionSystem:
    ...
    @staticmethod
    def _get_char_value(character, key: str, default=0):
        """
        统一获取角色属性值的辅助方法
        兼容字典和 Peewee 模型对象
        """
        if hasattr(character, '__data__'):
            # Peewee 模型对象
            return character.__data__.get(key, default)
        elif isinstance(character, dict):
            # 字典
            return character.get(key, default)
        else:
            # 尝试直接属性访问
            return getattr(character, key, default)
    ...
    def check_evolution(character) -> Tuple[bool, Optional[int], Optional[Dict]]:
        ...
        Args:
            character: 角色数据 (可以是字典或 Peewee 模型对象)

        Returns:
            (是否可以进化, 新阶段, 阶段信息)
        """
        current_stage = EvolutionSystem._get_char_value(character, "evolution_stage", 1)
        ...
        for attr, required in requirements.items():
            char_value = EvolutionSystem._get_char_value(character, attr, 0)
    ...
    @staticmethod
    def get_evolution_progress(character) -> Tuple[float, str]:
        """
        计算进化进度百分比和可视化进度条
    ...
    @staticmethod
    def get_next_stage_hint(character) -> str:
        """
        获取下一阶段的提示
