...
class ActionHandler:
    ...
    async def execute_action(
        ...
        from ..mechanics.delayed_consequence_system import DelayedConsequenceSystem
        ...
        if "mood_required" in action_config:
            from ..personality.dynamic_mood_system import DynamicMoodSystem
        ...
        if cooldown_seconds > 0:
            from ..time.cooldown_manager import CooldownManager
        ...
        if action_config.get("requires_confirmation", False):
            from ..mechanics.confirmation_manager import ConfirmationManager
...
""".strip()
        ...
        # === 6.2. 【新增】应用调教进度修正 ===
        from ..actions.training_progress_system import TrainingProgressSystem
        ...
        # === 6.4. 【新增】应用场景效果倍率 ===
        from ..scenes.enhanced_scene_system import EnhancedSceneSystem
        ...
        # === 6.8. 【新增】应用属性冲突机制 ===
        from ..attributes.attribute_conflict_system import AttributeConflictSystem
        ...
        # 9. 检查特殊事件触发
        from ..mechanics.scenario_engine import ScenarioEngine
        ...
        if success_llm and ai_response:
            # === 【优化】合并所有输出为一条消息 ===
            from ..personality.dual_personality_system import DualPersonalitySystem
            from ..events.post_action_events import PostActionEventSystem
        ...
        # 12.5. 检查进化阶段升级
        from ..relationship.evolution_system import EvolutionSystem
...
"""
        ...
        # === 【新增】检查关系张力危机 ===
        from ..relationship.relationship_tension_system import RelationshipTensionSystem
...
━━━━━━━━━━━━━━━━━━━"""
        ...
        # === 【新增】检查选择困境事件（完全动态生成） ===
        from ..events.choice_dilemma_system import ChoiceDilemmaSystem

        # 【新增】尝试使用 LLM 完全动态生成困境
        # 获取最近历史用于生成上下文
        history = await PromptBuilder.get_recent_history(user_id, chat_id, limit=3)

        # 30%概率触发困境
        if random.random() < 0.3:
            dynamic_dilemma = await ChoiceDilemmaSystem.generate_dynamic_dilemma(
                character=character,
                history=history
            )
            ...
            if dynamic_dilemma:
                # 成功生成动态困境
                # 【修复】存储完整困境数据到角色数据，而不仅仅是ID
                character["pending_dilemma"] = json.dumps({
                    "dilemma_id": dynamic_dilemma['dilemma_id'],
                    "dilemma_data": dynamic_dilemma  # 完整的困境数据
                }, ensure_ascii=False)
                ...
                logger.info(f"触发完全动态困境: {user_id} - {dynamic_dilemma['dilemma_name']}")

                # 构建困境消息
                dilemma_message = f"""{dynamic_dilemma['title']}

{dynamic_dilemma['description']}
...
你的选择:"""
                ...
                for idx, choice in enumerate(dynamic_dilemma['choices'], 1):
                    dilemma_message += f"\n\n{idx}. 【{choice['text']}】"
                    dilemma_message += f"\n   {choice['description']}"
                ...
                # 动态生成失败，回退到预定义困境
                logger.warning("动态困境生成失败，回退到预定义困境")
                ...
                if dilemma_triggered and dilemma_data:
                    ...
                    # 调用 LLM 生成困境文本
                    generated_dilemma = await ChoiceDilemmaSystem.generate_dilemma_content_with_llm(
                        dilemma_id=dilemma_id,
                        dilemma_data=ChoiceDilemmaSystem.DILEMMA_EVENTS.get(dilemma_id, {}),
                        character=character,
                        history=history
                    )

                    # 如果生成成功，合并生成的内容；否则使用原始固定文本
                    if generated_dilemma:
                        dilemma_data = ChoiceDilemmaSystem.merge_generated_dilemma(dilemma_data, generated_dilemma)
                        logger.info(f"使用LLM生成的困境内容: {dilemma_id}")
                    else:
                        logger.warning(f"LLM生成失败，使用固定文本: {dilemma_id}")
