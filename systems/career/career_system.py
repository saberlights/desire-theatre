...
class CareerSystem:
    ...
    CAREERS = {
        ...
        "high_school_student": {
            ...
            "stage_requirements": {
                "college_student": {"game_day": 15, "intelligence": 30},
        ...
        "college_student": {
            ...
            "stage_requirements": {
                "office_worker": {"game_day": 25, "professionalism": 20},
                "freelancer": {"game_day": 25, "creativity": 30},
                "graduate_student": {"game_day": 25, "intelligence": 50},
            },
        ...
        "office_worker": {
            ...
            "stage_requirements": {
                "senior_staff": {"game_day": 30, "professionalism": 40},
                "manager": {"game_day": 35, "professionalism": 60, "leadership": 30},
            },
        ...
        "senior_staff": {
            ...
            "stage_requirements": {
                "manager": {"game_day": 38, "leadership": 50},
                "specialist": {"game_day": 38, "professionalism": 70},
            },
        ...
        "freelancer": {
            ...
            "stage_requirements": {
                "successful_freelancer": {"game_day": 30, "creativity": 50},
                "entrepreneur": {"game_day": 35, "creativity": 60, "leadership": 40},
            },
        ...
        "successful_freelancer": {
            ...
            "stage_requirements": {
                "entrepreneur": {"game_day": 40, "creativity": 70, "leadership": 50},
            },
        ...
        "idol": {
            ...
            "stage_requirements": {
                "top_idol": {"game_day": 35, "charm": 60, "popularity": 70},
            },
        ...
        "model": {
            ...
            "stage_requirements": {
                "top_model": {"game_day": 32, "charm": 65, "popularity": 60},
            },
        ...
        "streamer": {
            ...
            "stage_requirements": {
                "top_streamer": {"game_day": 28, "charm": 50, "popularity": 55},
            },
        ...
        "part_time_worker": {
            ...
            "stage_requirements": {
                "hostess": {"game_day": 15, "charm": 30},
                "office_worker": {"game_day": 20, "professionalism": 25},
            },
        ...
        "hostess": {
            ...
            "stage_requirements": {
                "high_class_escort": {"game_day": 25, "charm": 60},
            },
...
""".strip()
    ...
    def get_career_display(character: Dict) -> str:
        ...
        if can_promote:
            ...
            for attr, required in requirements.items():
                ...
                if char_value < required:
                    ...
                    attr_name = attr_display_map.get(attr, CareerSystem.CAREER_ATTRIBUTES.get(attr, attr))
                    progress_lines.append(f"  {attr_name}: {char_value}/{required}")
        ...
        if category == "student":
            ...
            professionalism = character.get("professionalism", 0)
            leadership = character.get("leadership", 0)
            attr_display += f"\n专业度: {professionalism}/100"
            if career_id in ["senior_staff", "manager"]:
                attr_display += f"\n领导力: {leadership}/100"
        elif category == "freelance":
            creativity = character.get("creativity", 0)
            leadership = character.get("leadership", 0)
            attr_display += f"\n创造力: {creativity}/100"
            if career_id == "successful_freelancer":
                attr_display += f"\n领导力: {leadership}/100"
        elif category == "entertainment":
            charm = character.get("charm", 0)
            popularity = character.get("popularity", 0)
        ...
        if category == "freelance":
            creativity = character.get("creativity", 0)
            bonus_percent = int((creativity / 100) * 50)
            if bonus_percent > 0:
                income_bonus_hint = f"\n💡 创造力提升收入+{bonus_percent}%"
        elif category == "entertainment":
            charm = character.get("charm", 0)
            popularity = character.get("popularity", 0)
            bonus_percent = int(((charm + popularity) / 200) * 80)
            if bonus_percent > 0:
                income_bonus_hint = f"\n💡 魅力&人气提升收入+{bonus_percent}%"
        elif category == "career":
            professionalism = character.get("professionalism", 0)
            bonus_percent = int((professionalism / 100) * 30)
            if bonus_percent > 0:
                income_bonus_hint = f"\n💡 专业度提升收入+{bonus_percent}%"
...
💰 每日收入: {daily_income}币{income_bonus_hint}
📅 从业天数: {career_day}天
{attr_display}
{promotion_hint}
""".strip()
    ...
    @staticmethod
    def daily_career_growth(character: Dict) -> Tuple[Dict, List[str]]:
        """
        每日职业成长 - 根据职业类型自动提升相关属性
        ...
        # 根据职业类别增长相关属性
        # 基础成长值：每天+2，从业天数每10天额外+1
        career_day = character.get("career_day", 0)
        base_growth = 2
        bonus_growth = career_day // 10  # 每工作10天，成长速度+1

        total_growth = base_growth + bonus_growth

        if category == "student":
            # 学生增长智力
            character, msg = CareerSystem.train_attribute(character, "intelligence", total_growth)
            growth_messages.append(msg)

        elif category == "career":
            # 职场增长专业度
            character, msg = CareerSystem.train_attribute(character, "professionalism", total_growth)
            growth_messages.append(msg)

            # 资深职员和经理额外增长领导力
            if career_id in ["senior_staff", "manager"]:
                character, msg = CareerSystem.train_attribute(character, "leadership", max(1, total_growth // 2))
                growth_messages.append(msg)

        elif category == "freelance":
            # 自由职业增长创造力
            character, msg = CareerSystem.train_attribute(character, "creativity", total_growth)
            growth_messages.append(msg)

            # 成功的自由职业者额外增长领导力（为创业做准备）
            if career_id == "successful_freelancer":
                character, msg = CareerSystem.train_attribute(character, "leadership", max(1, total_growth // 2))
                growth_messages.append(msg)
            ...
            # 娱乐行业增长魅力和人气
            character, msg1 = CareerSystem.train_attribute(character, "charm", total_growth)
            character, msg2 = CareerSystem.train_attribute(character, "popularity", total_growth)
            growth_messages.append(msg1)
            growth_messages.append(msg2)

        elif category == "adult":
            # 成人职业增长魅力
            character, msg = CareerSystem.train_attribute(character, "charm", total_growth)
            growth_messages.append(msg)

        elif category == "service":
            # 服务行业增长专业度（较低）
            character, msg = CareerSystem.train_attribute(character, "professionalism", max(1, total_growth // 2))
            growth_messages.append(msg)

            # 兼职工也缓慢增长魅力
            if career_id == "part_time_worker":
                character, msg = CareerSystem.train_attribute(character, "charm", 1)
                growth_messages.append(msg)

        # 所有职业都缓慢增长自信
        if career_day > 0 and career_day % 3 == 0:  # 每3天增长1点自信
            character, msg = CareerSystem.train_attribute(character, "confidence", 1)
            growth_messages.append(msg)

        return character, growth_messages
